...
class BezierCurve < Struct.new(:x_range, :y_range, :pull)
  def self.create(x_range, y_range, pull = 0.0)
  ...
  class SimpleBezirCurve
    ...
    def intersection(p0, p1, p2, a, b, c)
      ...
      t = []
      if d > 0
        s = Math.sqrt(d) # !> assigned but unused variable - id
...
if $0 == __FILE__
  ...
  # 早熟
  curve1 = BezierCurve.create(1..9, 100..999, 0.25)
  ...
  # 普通
  curve2 = BezierCurve.create(1..9, 100..999)
  ...
  # 晩成
  curve3 = BezierCurve.create(1..9, 100..999, -0.25)
  ...
  records = (0..50).collect do |level|
    ...
    patterns.each do |e|
      if e[:level].include?(level)
        attrs[e[:name]] = BezierCurve.create(e[:level], e[:attack], e[:pull]).y_by_x(level)
      end
    end
    attrs
  end
  ...
  # patterns.each do |e|
  #   e[:level].collect{|x| {:level => x, e[:name] => BezierCurve.create(e[:level], e[:attack], e[:pull]).y_by_x(x)} }
  # end
  ...
  Gnuplot.open do |gp|
    Gnuplot::Plot.new(gp) do |plot|
      ...
      plot.data = patterns.collect do |e|
        x = e[:level].to_a
        y = e[:level].collect{|level| BezierCurve.create(e[:level], e[:attack], e[:pull]).y_by_x(level) }
